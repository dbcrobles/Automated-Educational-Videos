...
from .models import (
    ClaimSourceModel, ClaimModel, ChartPointModel, DataPointModel,
    ChartSpecModel, ElementModel, BeatModel, _strip_none, _coerce,
)
...
# Pipeline stage order per format, for the progress bar
PIPELINE_STAGES = {
    "long": [
        "Pending_Research", "QA_Research", "Pending_BeatScript",
        "QA_BeatScript", "Pending_Storyboard", "QA_Storyboard",
        "Awaiting_Narration", "Pending_LongRender", "QA_Final",
        "Ready_To_Publish", "Published",
    ],
    "short": [
        "Pending_Script", "QA_Script", "Awaiting_Narration",
        "Pending_Assets", "Pending_Render", "QA_Final",
        "Ready_To_Publish", "Published",
    ],
    "short_derived": [
        "Pending_ShortRender", "QA_Final", "Ready_To_Publish", "Published",
    ],
}
...
STATUS_META = {
    ...
    "Pending_ShortRender":{"label": "Short Render…",   "color": "purple"},
...
def _video_format(video_id):
    conn = database.get_connection()
    row = conn.execute("SELECT format FROM videos WHERE id = ?", (video_id,)).fetchone()
    conn.close()
    return (row[0] if row and row[0] else 'short')
...
class VideoModel(BaseModel):
    ...
    artifact_origin: str = ""
    artifact_claims: list[ClaimModel] = []
    artifact_data_points: list[DataPointModel] = []
    use_degraded_model: bool = False
    beat_script_json: str = ""
    beat_script_beats: list[BeatModel] = []
    beat_script_title: str = ""
    beat_script_duration: float = 0.0
    beats_json_str: str = ""
    storyboard_beats: list[BeatModel] = []
...
class State(LongFormQAMixin, FinalQAMixin, rx.State):
    ...
    # Filler short creation (tucked-away legacy pipeline)
    short_form_topic: str = ""
    show_short_creator: bool = False

    @rx.event
    def set_short_form_topic(self, val: str):
        self.short_form_topic = val

    @rx.event
    def toggle_short_creator(self):
        self.show_short_creator = not self.show_short_creator

    def add_short_video(self):
        """Filler short: fully automated legacy pipeline (AI script + stock assets)."""
        if not self.short_form_topic:
            return
        conn = database.get_connection()
        conn.execute(
            "INSERT INTO videos (topic, account_id, format, status) VALUES (?, ?, 'short', 'Pending_Script')",
            (self.short_form_topic, self.new_category))
        conn.commit()
        conn.close()
        self.short_form_topic = ""
        self.load_videos()
        return rx.toast.success("Filler short queued — AI scripting will start shortly.")
    ...
    def load_videos(self):
        ...
        def _stage_pct(status, fmt):
            stages = PIPELINE_STAGES.get(fmt) or PIPELINE_STAGES["short"]
            if status in stages:
                return round((stages.index(status) + 1) * 100 / len(stages))
            return 0
        ...
        for row in rows_dicts:
            ...
            videos.append(VideoModel(
                ...
                stage_pct=_stage_pct(row['status'], row.get('format') or 'short'),
                ...
                artifact_claims=_coerce(ClaimModel, art_claims),
                artifact_data_points=_coerce(DataPointModel, art_dps),
                use_degraded_model=bool(row.get('use_degraded_model', 0)),
                beat_script_json=row.get('beat_script') or "",
                beat_script_beats=_coerce(BeatModel, bs_beats),
                beat_script_title=bs_title,
                beat_script_duration=bs_duration,
                beats_json_str=row.get("beats_json") or "",
                storyboard_beats=_coerce(BeatModel, realized_beats),
                narration_beats=_coerce(NarrationBeatModel, narration_beats),
                **final_qa_view(row),
            ))
        self.videos = videos
    ...
    def derive_short(self, parent_id: int, beat_order: int):
        """Phase 8: derive a vertical short from one beat of a published long-form
        video. No LLM calls — the beat already carries narration timing, the
        hook_label, and realized element specs in the parent's beats_json."""
        conn = database.get_connection()
        conn.row_factory = database.sqlite3.Row
        parent = conn.execute("SELECT * FROM videos WHERE id = ?", (parent_id,)).fetchone()
        conn.close()
        if not parent:
            return rx.toast.error("Parent video not found.")
        parent = dict(parent)
        beat = next((b for b in parse_storyboard_beats(parent.get('beats_json'))
                     if b.get('order') == beat_order), None)
        if not beat:
            return rx.toast.error(f"Beat {beat_order} not found in the parent's beats.json.")
        topic = f"{parent['topic']} — {beat.get('hook_label') or f'Beat {beat_order}'}"
        conn = database.get_connection()
        conn.execute(
            "INSERT INTO videos (topic, account_id, format, status, parent_video_id, "
            "beat_order, cta_text, is_sponsored, save_to_desktop, post_yt, post_ig, post_tt) "
            "VALUES (?, ?, 'short_derived', 'Pending_ShortRender', ?, ?, ?, ?, ?, ?, ?, ?)",
            (topic, parent['account_id'], parent_id, beat_order,
             parent.get('cta_text'), parent.get('is_sponsored') or 0,
             parent.get('save_to_desktop') if parent.get('save_to_desktop') is not None else 1,
             parent.get('post_yt') if parent.get('post_yt') is not None else 1,
             parent.get('post_ig') if parent.get('post_ig') is not None else 1,
             parent.get('post_tt') if parent.get('post_tt') is not None else 1))
        conn.commit()
        conn.close()
        self.load_videos()
        return rx.toast.success(f"Short queued from beat {beat_order}.")
    ...
    def retry_video(self, video_id: int):
        """Smart retry: resume from the furthest stage that already has valid
        output on disk. Format-aware — long-form resumes along the beat
        pipeline, filler shorts along the legacy pipeline, derived shorts
        simply re-render."""
        conn = database.get_connection()
        conn.row_factory = database.sqlite3.Row
        cursor = conn.cursor()
        ...
        video = dict(row)
        fmt = video.get('format') or 'short'
        vpath = video.get('voiceover_path') or ''

        if fmt == 'short_derived':
            resume_status = 'Pending_ShortRender'
        elif fmt == 'long':
            resume_status = 'Pending_Research'
            if video.get('research_artifact'):
                resume_status = 'Pending_BeatScript'
            if video.get('beat_script'):
                resume_status = 'Pending_Storyboard'
            if video.get('beats_json'):
                resume_status = 'Awaiting_Narration'
            if vpath and os.path.exists(vpath):
                resume_status = 'Pending_LongRender'
        else:
    ...
    def retry_from_scratch(self, video_id: int):
        """Hard reset — long-form restarts at research, shorts at scripting."""
        note = str(self.rejection_notes.get(video_id, "")).strip()
        fmt = _video_format(video_id)
        updates = {
            'status': 'Pending_Research' if fmt == 'long' else 'Pending_Script',
            'error_message': None,
            'research_dossier': None, 'research_artifact': None,
            'storyboard_draft': None, 'beat_script': None, 'beats_json': None,
    ...
    def reject_to_script(self, video_id: int):
        """Rewrite the script/beats while preserving the expensive research checkpoint."""
        note = str(self.rejection_notes.get(video_id, "")).strip()
        fmt = _video_format(video_id)
        if fmt == 'long':
            updates = {'status': 'Pending_BeatScript', 'error_message': None,
    ...
    def reject_to_render(self, video_id: int):
        """Send back to Node 4 (render) with editor note — cheapest retry."""
        note = str(self.rejection_notes.get(video_id, "")).strip()
        fmt = _video_format(video_id)
        target = {'long': 'Pending_LongRender',
                  'short_derived': 'Pending_ShortRender'}.get(fmt, 'Pending_Render')
        updates = {'status': target, 'error_message': None}
        if note:
            updates['qa_feedback'] = note
        database.update_video(video_id, updates)
        self.rejection_notes = {k: v for k, v in self.rejection_notes.items() if k != video_id}
    ...
    def reject_to_assets(self, video_id: int):
        ...
        database.update_video(video_id, {
            'status': 'Pending_Storyboard' if fmt == 'long' else 'Pending_Assets',
    ...
    async def handle_upload(self, files: list[rx.UploadFile]):
        ...
        for file in files:
            ...
            outfile = os.path.join(assets_dir, file.filename or "intro.mp4")
